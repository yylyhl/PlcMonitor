    ...
    public static class LoggingSerilogExtensions
    {
        ...
        private static LoggerConfiguration DefaultConfig()
        {
            return new LoggerConfiguration()
                ...
                //.WriteTo.Console(outputTemplate:"[{Timestamp:HH:mm:ss} {Level:u3}] {ThreadId} {SourceContext} | {Message:lj}{NewLine}{Exception}{NewLine}")
                .WriteTo.Async(a => a.File(
                    ...
                    //outputTemplate: "{Timestamp:HH:mm:ss.fff} [{TraceId}] [{Level:u3}] Thread:{ThreadId} {SourceContext}: {Properties} {Message}{NewLine}{Exception}{NewLine}"
                    outputTemplate: "{Timestamp:HH:mm:ss.fff} [{Level:u3}] Thread:{ThreadId} {SourceContext}: {Properties} {Message}{NewLine}{Exception}{NewLine}"
                ))
                .WriteTo.Async(a => a.File(
                    //formatter: new Serilog.Formatting.Json.JsonFormatter(),
                    path: "logs/warn-.txt",
                    ...
                    flushToDiskInterval: TimeSpan.FromSeconds(2),
                    encoding: new System.Text.UTF8Encoding(encoderShouldEmitUTF8Identifier: true),
                    //outputTemplate: "{Timestamp:HH:mm:ss.fff} [{TraceId}] [{Level:u3}] Thread:{ThreadId} {SourceContext}: {Properties} {Message}{NewLine}"
                    outputTemplate: "{Timestamp:HH:mm:ss.fff} [{Level:u3}] Thread:{ThreadId} {SourceContext}: {Properties} {Message}{NewLine}"
                ...
                .WriteTo.Async(a => a.File(
                    ...
                    flushToDiskInterval: TimeSpan.FromSeconds(2),
                    encoding: new System.Text.UTF8Encoding(encoderShouldEmitUTF8Identifier: true),
                    //outputTemplate: "{Timestamp:HH:mm:ss.fff} [{TraceId}] [{Level:u3}] Thread:{ThreadId} {SourceContext}: {Message}{NewLine}{Exception}{NewLine}"
